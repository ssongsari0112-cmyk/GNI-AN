...
- Activity: 각 Output의 children에 반드시 3~4개 (최소 3개. 실행 가능한 구체 활동. 절대 1~2개만 만들지 말 것)
...
[문체 — narrative/indicators/verificationMeans/assumptions 모두 적용]
- 간결체·명사형 종결로 작성 ("~함", "~음", "~됨" 또는 명사 자체로 끝맺기)
- "~합니다", "~한다", "~하고자 합니다" 같은 서술형/구어체 종결 금지
  ❌ "교육을 실시합니다" / "체계를 구축한다"  →  ✅ "교육 실시" / "체계 구축"
...
- 각 출처마다 items 배열로 "투입 항목명"만 작성 (구체적인 수량·금액·명수를 추측해서 쓰지 말 것 —
  실제 담당자가 추후 직접 입력할 항목 카테고리만 제시):
  - 예산 항목: 숫자를 절대 추측하지 말고 반드시 "3년간 사업비 약 OO원" 문자열을 그대로 사용 ("OO" 그대로)
  - 인력 항목: 구체적 인원수·기간을 추측하지 말고 역할명만 제시 (예: "사업 총괄 책임자(Project Manager) 파견",
    "현지 수행인력(Project Officer) 파견" — "OO명", "OO개월" 같은 수치 추측 금지)
  - 행정·협력 체계: "정부관계자 협조체계 구축" 등 항목명만, 구체적 횟수·수치 금지
...
export async function POST(req: NextRequest) {
  try {
    ...
    const contextLines = [
      ...
      ctx.problemTree         && `문제나무 구조 (최신 사용자 편집 반영, 빠짐없이 참고): ${ctx.problemTree}`,
      ctx.objectiveTree       && `목표나무 구조 (최신 사용자 편집 반영, 빠짐없이 참고): ${ctx.objectiveTree}`,
    ].filter(Boolean).join('\n');
...
- Activity 각 Output당 반드시 3~4개: 실행 가능한 구체 활동 (추상 표현 금지, 절대 1~2개만 만들지 말 것)
- Inputs: 최소 3개 출처(KOICA/한국 수행기관 본부/현지사무소·파트너), 출처당 최소 2개 항목
  (항목명만 작성하고 구체적 수량·금액·명수는 추측하지 말 것. 예산은 "3년간 사업비 약 OO원" 문자열을 그대로 사용)
