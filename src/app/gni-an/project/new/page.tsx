...
const PROGRAMS = [
  ...
  {
    id: 'special',
    icon: <Lightbulb size={28} className="text-[#8AA81E]" />,
    label: '특수사업(CTS, IBS)',
  ...
  return (
    <div className="min-h-screen bg-[#F7F8F2]">
      <header className="border-b border-[#D9E6B7] bg-white/80 backdrop-blur-sm sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 lg:px-10 h-16 flex items-center gap-3">
          <Link href="/gni-an" className="flex items-center gap-1.5 text-gray-500 hover:text-[#8AA81E] transition-colors">
            <ArrowLeft size={18} />
            홈으로
          </Link>
          <ChevronRight size={16} className="text-gray-300" />
          <span className="font-medium text-[#111827]">새 프로젝트</span>
        </div>
      </header>

      <main className="max-w-7xl mx-auto px-6 lg:px-10 py-10">
        <div className="text-center mb-10">
          <h1 className="text-4xl font-bold text-[#111827] mb-3">어떤 프로그램을 준비하시나요?</h1>
          <p className="text-gray-500 text-lg">사업 유형을 선택하면 적합한 공모 기준과 가이드를 제공합니다.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {PROGRAMS.map((prog) => (
            <div
              ...
              className={`relative bg-white border rounded-2xl p-8 transition-all flex flex-col ${
                prog.active
            ...
            >
              ...
              <div className="w-16 h-16 rounded-xl bg-[#EEF5D6] flex items-center justify-center mb-5">
                {prog.icon}
              </div>

              <h3 className="font-bold text-[#111827] text-xl mb-2">{prog.label}</h3>
              <p className="text-gray-500 leading-relaxed mb-4">{prog.desc}</p>
